# Agent operating record

- Date/time:
- Accountable owner:
- Operator:
- Agent home and definition digest:
- Bench suite / Agent version:
- Operation: run / tick / inspect / act / learn / amend / retire
- Checkpoint or unit of work:
- Granted authority and budgets:
- Explicitly absent authority:

## Result

- Command or managed operation:
- Exit/status:
- Terminal reason:
- Stdout/result artifact:
- Stderr/controller evidence:
- External case/pilot measure:
- Pending decision:
- Unknown effect or state:
- Next safe action and owner:
